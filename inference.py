...
def load_label_maps(cfg: AutoConfig):
...
def main():
    ...
    parser.add_argument("--model_id", default="outputs/outputs_mobilenetv3_large", help="HF model repo or local path")
    ...
    # Carga processor y modelo (no vuelvas a crearlos en run_batch)
...
def run_batch(
    ...
    # SOLO processor sobre PIL: que lea size/mean/std del preprocessor_config.json
    imgs = [im for _, im in batch]
    inputs = processor(images=imgs, return_tensors="pt")   # no ToTensor manual
