...
from torchvision.transforms import (
    CenterCrop, Compose, Lambda, Normalize, RandomHorizontalFlip, RandomResizedCrop,
    Resize, ToTensor, RandomRotation, RandomAffine, RandomVerticalFlip, RandomPerspective
)
from transformers import (
    AutoConfig, AutoImageProcessor, AutoModelForImageClassification, SchedulerType, get_scheduler
)
...
def parse_args():
    ...
    parser.add_argument("--cv_folds", type=int, default=0, help="K para cross validation, 0/1 = sin CV")
    parser.add_argument("--group_column_name", type=str, default=None, help="Columna de grupos para StratifiedGroupKFold (opcional)")
    ...
    parser.add_argument("--image_column_name", type=str, default="image", help="The name of the dataset column containing the image data. Defaults to 'image'.")
    parser.add_argument("--train_val_split", type=float, default=None, help="Percent to split off of train for validation")
    parser.add_argument("--max_train_steps", type=int, default=None, help="Total number of training steps to perform. If provided, overrides num_train_epochs.")
    parser.add_argument("--max_train_samples", type=int, default=None, help=(
        "For debugging purposes or quicker training, truncate the number of training examples to this "
        "value if set."
    ))
    parser.add_argument("--max_eval_samples", type=int, default=None, help=(
    ...
    parser.add_argument("--label_column_name", type=str, default="label", help="The name of the dataset column containing the labels. Defaults to 'label'.")
    parser.add_argument("--model_name_or_path", type=str, default="google/vit-base-patch16-224-in21k", help="Path to pretrained model or model identifier from huggingface.co/models.")
    ...
    parser.add_argument("--push_to_hub_model_id", type=str, default=None, help="El nombre del repositorio para sincronizar con el directorio local output_dir.")
    ...
    parser.add_argument("--do_eval", action="store_true", help="Si se especifica, ejecuta la evaluación del modelo.")
    parser.add_argument("--logging_strategy", type=str, default="steps", choices=["steps", "epoch", "no"], help="The logging strategy to adopt during training.")
    parser.add_argument("--logging_steps", type=int, default=10, help="Number of update steps between two logs if logging_strategy='steps'")
    parser.add_argument("--eval_strategy", type=str, default="epoch", choices=["steps", "epoch", "no"], help="The evaluation strategy to adopt during training.")
    parser.add_argument("--save_strategy", type=str, default="epoch", choices=["steps", "epoch", "no"], help="The checkpoint save strategy to adopt during training.")
    parser.add_argument("--load_best_model_at_end", type=str, default="false", help="Whether to load the best model at the end of training (true/false)")
    parser.add_argument("--save_total_limit", type=int, default=None, help="If a value is passed, limit the total amount of checkpoints. Deletes the older checkpoints.")
    parser.add_argument("--num_images_to_log", type=int, default=10, help="Number of images to log during evaluation.")
    args = parser.parse_args()
    if args.dataset_name is None and getattr(args, "train_dir", None) is None and getattr(args, "validation_dir", None) is None:
...
def main():
    ...
    send_example_telemetry("cacao-hf4", args)
    ...
    if accelerator.is_main_process:
        if args.push_to_hub:
            repo_name = args.push_to_hub_model_id or Path(args.output_dir).absolute().name
            api = HfApi()
    ...
    if args.dataset_name is not None:
        ...
        data_files = {}
        if getattr(args, "train_dir", None) is not None:
            data_files["train"] = os.path.join(args.train_dir, "**")
        if getattr(args, "validation_dir", None) is not None:
    ...
    if args.with_tracking:
        project_name = "cocoa-image-classification-with-cv"
        cfg = vars(args).copy()
        cfg["lr_scheduler_type"] = str(cfg["lr_scheduler_type"])
        cfg["cv_folds"] = args.cv_folds
        accelerator.init_trackers(project_name, config=cfg)
    use_cv = args.cv_folds and args.cv_folds > 1
    if use_cv:
        base_train = dataset["train"]
        y = np.array(base_train[args.label_column_name])
        if args.group_column_name and args.group_column_name in base_train.column_names:
            groups = np.array(base_train[args.group_column_name])
            splitter = StratifiedGroupKFold(n_splits=args.cv_folds, shuffle=True, random_state=args.seed or 42)
            folds = list(splitter.split(np.zeros(len(y)), y, groups))
        else:
            splitter = StratifiedKFold(n_splits=args.cv_folds, shuffle=True, random_state=args.seed or 42)
    ...
    with accelerator.main_process_first():
        ...
        if args.max_eval_samples is not None and "test" in dataset:
            dataset["test"] = dataset["test"].shuffle(seed=args.seed).select(range(args.max_eval_samples))
    _base_output_dir = args.output_dir
    for fold_idx in range(len(folds)):
        if use_cv:
            tr_idx, va_idx = folds[fold_idx]
            base_train = dataset["train"]
            train_dataset = base_train.select(tr_idx).with_transform(preprocess)
            eval_dataset = base_train.select(va_idx).with_transform(preprocess_val)
        else:
            train_dataset = dataset["train"].with_transform(preprocess)
            val_split = "validation" if "validation" in dataset else "test"
            print(f"Original validation dataset size: {len(dataset[val_split])}")
            eval_dataset = dataset[val_split].with_transform(preprocess_val)
        if _base_output_dir is not None:
            args.output_dir = os.path.join(_base_output_dir, f"fold_{fold_idx}") if use_cv else _base_output_dir
            os.makedirs(args.output_dir, exist_ok=True)
        def collate_fn(examples):
            pixel_values = torch.stack([example["pixel_values"] for example in examples])
            labels_tensor = torch.tensor([example[args.label_column_name] for example in examples])
        ...
        global_metrics = evaluate.combine([metric_accuracy, metric_specificity])
        per_class_metrics = evaluate.combine([metric_precision, metric_recall])
        total_batch_size = args.per_device_train_batch_size * accelerator.num_processes * args.gradient_accumulation_steps
        logger.info(f"***** Fold {fold_idx} - Running training *****")
        ...
        for epoch in range(starting_epoch, args.num_train_epochs):
            ...
            if args.do_eval:
                model.eval()
                all_predictions, all_references, all_images = [], [], []
                eval_loss_total, eval_steps = 0.0, 0
                for step, batch in enumerate(eval_dataloader):
                    with torch.no_grad():
                        outputs = model(**batch)
                        eval_loss_total += outputs.loss.detach().float().item()
                        eval_steps += 1
                        predictions = outputs.logits.argmax(dim=-1)
                        predictions, references = accelerator.gather_for_metrics((predictions, batch["labels"]))
                        per_class_metrics.add_batch(predictions=predictions, references=references)
                        global_metrics.add_batch(predictions=predictions, references=references)
                ...
                eval_loss_value = eval_loss_total / max(1, eval_steps)
                train_loss_value = total_loss.item() / len(train_dataloader)
                if args.with_tracking and 'wandb' in args.report_to:
                    ...
                    metrics_dict = {
                        ...
                        "cv_fold": fold_idx if use_cv else -1,
                    }
                    metrics_dict.update({
