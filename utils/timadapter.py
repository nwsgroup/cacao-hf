import os
from typing import Optional, Union, Tuple, OrderedDict
...
from transformers.models.vit.configuration_vit import ViTOnnxConfig
...
class TimmForImageClassification(TimmPreTrainedModel):
    ...
    @classmethod
    def from_pretrained(cls, model_name_or_path, config_hf, **kwargs):
        config = TimmConfig.from_pretrained(model_name_or_path, **kwargs)

        for attr in config_hf.__dict__:
            if hasattr(config, attr):
                setattr(config, attr, getattr(config_hf, attr))
